...
from Oklogk import mobius_sieve, harmonic_numbers, mu_convolution_H
import numpy as np 
import math 
...
def enumerate_harmonic_oscillator(K=200, epochs=2, max_memory=64, seed=42):
    """
    Enumerate a harmonic oscillator in O(K log K) using step gates.
    1. Compute F = (μ * H)(n) for n=1..K (O(K log K)).
    2. Store F values at odd square‑free indices.
    3. For each epoch:
       - Apply Collatz index mapping.
       - Apply a logic gate (log or exp) to values.
       - Run chip compression (keeps top M coefficients based on supertrace).
    4. Return the final compressed memory.
    """
    # ---- Step 1: Compute the initial oscillator state ----
    F, mu, H = mu_convolution_H(K)   # F[1..K] computed in O(K log K)
    print(f"Initial F computed for n=1..{K}")

    # ---- Step 2: Store in Möbius memory (odd square‑free indices only) ----
    mem = MobiusCollatzMemory(max_index=K, use_square_free=True)
    # We'll use the first K odd square‑free indices to store F values.
    # But F is defined for all n; we only keep those where mu[n] != 0 and n odd.
    idx = 1
    count = 0
    while count < K and idx <= K:
        if mem._valid_index(idx):   # odd and square‑free
            # F[idx] is a real value; we can take absolute or keep as is.
            val = F[idx] if idx < len(F) else 0.0
            if abs(val) > 1e-12:
                mem.write(idx, val)
                count += 1
        idx += 2  # only odd
    print(f"Initial memory: {len(mem)} coefficients")

    # ---- Step 3: Apply step gates for each epoch ----
    gate = ChipLogicGate(max_K=K)   # for logic transforms
    processor = ChipProcessor(max_K=K)  # for chip compression

    for epoch in range(epochs):
        print(f"\nEpoch {epoch+1}:")
        # a) Extract current signal (sorted by index)
        items = sorted(mem.data.items())
        signal = np.array([val for _, val in items], dtype=float)
        print(f"  Signal length: {len(signal)}")

        # b) Apply a logic gate (alternate between log and exp)
        if epoch % 2 == 0:
            kept, S, H_ent, m, conv = gate.bounded_log(signal)
            print(f"  Applied log: S={S:.4f}, H={H_ent:.4f}, m={m:.4f}")
        else:
            kept, S, H_ent, m, conv = gate.bounded_exp(signal)
            print(f"  Applied exp: S={S:.4f}, H={H_ent:.4f}, m={m:.4f}")

        # c) Collatz index mapping: the `kept` indices are 0-based positions.
        # We need to map them to original odd square‑free indices.
        # Then apply the Collatz step to each original index.
        new_mem = MobiusCollatzMemory(max_index=K*3+1, use_square_free=True)
        for pos, val in kept:
            if pos < len(items):
                old_idx = items[pos][0]   # actual Möbius index
                # Collatz: n -> 3n+1, then divide by 2 until odd.
                new_idx = 3 * old_idx + 1
                while new_idx % 2 == 0:
                    new_idx //= 2
                # Only keep if odd and square‑free
                if new_mem._valid_index(new_idx):
                    # Optionally scale the value by ALPHA (as in earlier Collatz memory)
                    new_mem.write(new_idx, val * ALPHA)
        # Replace old memory with new one
        mem = new_mem
        print(f"  After Collatz step: {len(mem)} coefficients")

        # d) Optional: further compress using chip pipeline to stay within max_memory
        if len(mem) > max_memory:
            items = sorted(mem.data.items())
            signal = np.array([val for _, val in items], dtype=float)
            # Compress via chip pipeline (keeps only top M based on supertrace)
            kept_comp, S_c, H_c, m_c, conv_c = chip_pipeline(signal, processor=processor)
            new_mem = MobiusCollatzMemory(max_index=K*3+1, use_square_free=True)
            for pos, val in kept_comp:
                if pos < len(items):
                    idx_orig = items[pos][0]
                    if new_mem._valid_index(idx_orig):
                        new_mem.write(idx_orig, val)
            mem = new_mem
            print(f"  Compressed to {len(mem)} coefficients (≤ max_memory={max_memory})")

    return mem

# ---------- Main ----------
def main():
    print("=== Harmonic Oscillator Enumeration with Step Gates ===\n")
    K = 128   # number of harmonic points
    mem = enumerate_harmonic_oscillator(K=K, epochs=2, max_memory=64)

    print(f"\nFinal memory contains {len(mem)} coefficients.")
    print("First 10 stored entries (index, value):")
    for i, (idx, val) in enumerate(sorted(mem.data.items())):
        if i >= 10:
            break
        print(f"  {idx}: {val:.6f}")

    # Compute supertrace of final state
