...
class MobiusConverterApp:
    def __init__(self, root):
        ...
        self.converter = MobiusFileConverter(K=1, gate='log')

        # Variables
        self.input_file = tk.StringVar()
        self.output_file = tk.StringVar()
        self.K_val = tk.IntVar(value=1)
        self.gate_val = tk.StringVar(value='log')
        self.chunk_val = tk.IntVar(value=1)
        ...
        tk.Button(btn_frame, text="Compress → .mob", command=self.do_compress, bg='lightblue', width=20).pack(side=tk.LEFT, padx=5)
    ...
    def browse_input(self):
        ...
        if f:
            ...
            if not self.output_file.get():
                self.output_file.set(os.path.splitext(f)[0] + '.mob')
            self.update_preview()

    def browse_output(self):
        f = filedialog.asksaveasfilename(defaultextension=".mob")
        if f:
    ...
    def do_compress(self):
        ...
        def task():
            try:
                ...
                # Update preview with the new .mob file's fingerprint
    ...
    def do_decompress(self):
        if not self.input_file.get():
            messagebox.showerror("Error", "No input .mob file selected.")
            return
        if not self.output_file.get():
            messagebox.showerror("Error", "No output file specified.")
            return
